...
const REPORT_PATH = process.env.READINESS_REPORT_PATH || "reports/production-readiness.json";
const monitorSecret = process.env.HEALTH_MONITOR_SECRET || "";
const startedAt = Date.now();

const response = await fetch(`${BASE_URL}/api/readiness`, {
  headers: {
    "user-agent": "fizioterapia-ime-readiness-monitor/1.0",
...
const checks = {
  ...
  readyStatusCode: response.status === 200,
  readinessPayload: payload?.status === "ready",
  schemaVersionsMatch:
    typeof payload?.schemaVersion === "string" &&
    payload.schemaVersion === payload?.expectedSchemaVersion,
...
const report = {
  ...
  baseUrl: BASE_URL,
  endpoint: `${BASE_URL}/api/readiness`,
