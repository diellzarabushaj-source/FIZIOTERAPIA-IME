...
export default function ClinicalRecommendationsPage() {
  const [conditionSlug, setConditionSlug] = useState(conditions[0][0]);
  ...
  return (
    <main style={{ maxWidth: 1120, margin: "0 auto", padding: "32px 20px 80px" }}>
      <a href="/physiotherapist-portal" style={{ fontWeight: 800 }}>← Kthehu në portal</a>
      ...
      <section className="card" style={{ padding: 24, display: "grid", gap: 20 }}>
        ...
        <button className="button" onClick={submit} disabled={loading}>{loading ? "Duke llogaritur…" : "Gjenero rekomandimet"}</button>
      ...
      {result && (
        <section style={{ marginTop: 32 }}>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            {result.recommendations.length === 0 ? <p>Nuk u gjet asnjë ushtrim i lejuar me kriteret aktuale.</p> : result.recommendations.map((item) => <RecommendationCard key={item.exercise.id} item={item} />)}
          </div>
        </section>
      )}
    </main>
...
function RecommendationCard({ item }: { item: ExerciseRecommendation }) {
  return (
    <article className="card" style={{ padding: 20, display: "grid", gridTemplateColumns: "96px 1fr", gap: 18 }}>
      <img src={item.exercise.thumbnailUrl} alt="Ilustrim demonstrues i ushtrimit" width={96} height={96} style={{ width: 96, height: 96, objectFit: "cover", borderRadius: 14 }} />
      <div>
        ...
        <button className="button secondary" type="button" style={{ marginTop: 12 }}>Shto në draft-plan</button>
      </div>
    </article>
  );
}
