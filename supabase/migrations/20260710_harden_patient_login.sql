...
declare
  ...
  v_count integer;
begin
  delete from public.patient_login_attempts where created_at < now() - interval '24 hours';

  select count(*) into v_count
  from public.patient_login_attempts
  where success = false
    and created_at >= now() - interval '15 minutes'
    and (code_fingerprint = v_fingerprint or (v_ip is not null and ip_address = v_ip));
