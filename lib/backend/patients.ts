...
export async function archivePatientForActor(
  ...
  if (patientResult.data.archived_at) return fail("CONFLICT", "Pacienti është arkivuar tashmë.");
...
export async function restorePatientForActor(
  ...
  if (!patientResult.data.archived_at) return fail("CONFLICT", "Pacienti nuk është i arkivuar.");
